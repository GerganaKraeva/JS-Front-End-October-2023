function solve(input) {
    ...
    for (let currentAustronaut = 0; currentAustronaut < astronautsCount; currentAustronaut++) {
    ...
    while (input.lenght > 0) {
        ...
        switch (action) {
            ...
            case 'Breathe':
                ...
                const astronautToBreathe = astronauts.find(astronaut => astronaut.name === name);
                if (astronautToBreathe) {
                    const oxygenRecovered = Math.min(amountOxygen, 100 - astronautToBreathe.oxygen);
                    astronautToBreathe.oxygen += oxygenRecovered;
                    console.log(`${astronautToBreathe.name} took a breath and recovered ${oxygenRecovered} oxygen!`);
                }
                break;
        }
    }
